...
class OutputsAnimator():
    ...
    def main(self, start_date, length):
        ...
        save_dir = os.path.join(CONFIG['local']['json_path'], 'Animation')
        ...
        for single_date in daterange(start_date, end_date):
            ...
            self.fig.savefig(os.path.join(save_dir, filename), dpi=200)
            plt.close('all')
        #generating animation
        animation_path = os.path.join(save_dir,
                                      ("animation_" + CONFIG['local']['json_name'][:-5] + '.gif'))
